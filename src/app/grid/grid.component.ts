...
export class GridComponent  implements AfterViewInit, OnInit {
  ...
  addTile(data: Card) {    
    ...
    this.tileCount++;   
    var tile = { id: data.id, name: data.name, img: data.image_uris.art_crop, status: "enabled", wins: 0, losses: 0, colors: data.color_identity.join(',') } as CardTile;
  ...
  insertRow(tile: CardTile) {
    // db.insertNewTile(tile);
  }

  updateRowStatus(status: string, id: string) {
    // db.updateRowStatus(status, id);
  }
  ...
  clickListener(ev: any) {
      ...
      var x = document.getElementById(id);
        if(this.tiles[this.tiles.findIndex(f => f.id == id)].status == "disabled")
        {
          ...
          this.updateRowStatus("enabled", id);
        }
        else {
          x!.style.filter = "grayscale(100%)";
          this.tiles[this.tiles.findIndex(f => f.id == id)].status = "disabled";
          this.updateRowStatus("disabled", id);
        }
  }
